...
1. [操作前准备](#1-操作前准备)
2. [确认 TAG 分区](#2-确认-tag-分区)
3. [检查网络](#3-检查网络)
4. [清理临时文件并下载](#4-清理临时文件并下载)
5. [检查下载文件](#5-检查下载文件)
6. [赋予执行权限并确认工具](#6-赋予执行权限并确认工具)
7. [擦除并写入 TAG](#7-擦除并写入-tag)
8. [同步并完整读回](#8-同步并完整读回)
9. [把读回文件传回电脑](#9-把读回文件传回电脑)
10. [确认型号字符串](#10-确认型号字符串)
11. [校验通过后重启](#11-校验通过后重启)
12. [异常处理](#12-异常处理)
13. [禁止操作](#13-禁止操作)
14. [最终检查清单](#14-最终检查清单)
...
- 电脑 TFTP 根目录中放入 `mtd_write_tag_armv7`。
...
rm -f /tmp/mtd_write_tag /tmp/tag.bin /tmp/tag_readback.bin
