...
updated: 2026-09-14
description: 从 imsyy/home 出发重写导航主页并持续迭代到性能治理：Vue 3 + Vite 单依赖毛玻璃 Bento 布局、全配置驱动的二级面板、免 Key 天气；音乐播放器先是照搬 Firefly 的多源自愈设计，后来又接上洛雪音源（serverless 函数跑自定义音源脚本，解决 VIP 曲只能试听）；性能部分用 nvidia-smi 实测把「入场掉帧」「GPU 拉满」拆到根因（整屏 backdrop-filter 采样活跃页面、整屏 mix-blend-mode、opacity 动画暂停毛玻璃），附完整测量方法与踩坑记录
image: ./preview.jpg
tags: [Vue3, Vite, 个人主页, 前端, 性能优化, 洛雪音源, serverless]
category: 博客搭建
draft: false
pinned: false
...
> **音乐源：** 公共 Meting → 后来接上洛雪音源（serverless 函数跑自定义音源脚本，解决 VIP 曲只能试听）
> **部署：** GitHub + Vercel：静态站 + 一个 serverless 函数，零额外基础设施
...
这个播放器后来也成了性能问题的主角：410 行的歌单在前端是个不小的渲染负担，具体见第九节。

## 七、给播放器接上「洛雪音源」：从自建后端到 serverless

Meting 有个绕不过去的边界：**VIP / 版权受限曲目拿不到可播放直链**。而且它不是"报错"，是给一段**能播的 30 秒试听片段**——

| 同一首歌 | 实际文件 | 换算时长 |
| --- | --- | --- |
| Meting 给 | 721,023 字节 | 约 30 秒试听片段 |
| 洛雪音源给 | 9,720,729 字节 | 242 秒完整歌曲 |

探针只看"能不能拿到 metadata"，试听片段当然能，于是它赢下竞速、锁死播放位置；播到十几秒没声了也不报错，降级链永远触发不了。表现就是"后端能放、前端只能试听"。

### 7.1 两代方案

**第一代：自建 Go 后端。** 洛雪音源是 JS 脚本（洛雪客户端的自定义源格式），浏览器里跑不了（CORS + 缺宿主），所以放到服务器：Go 单二进制里用 goja 当 JS 引擎，把 `lx.on / lx.request / lx.utils` 这套宿主契约实现出来。实测 21 个流传的音源脚本只装得上 11 个——缺 `console`、`setTimeout`、`fetch`、`Buffer`、`atob`、`process` 这些标准全局；补齐后 20 个。

**第二代：serverless（当前方案）。** 换到 Vercel 函数后反而更省事：**音源脚本本来就是 JS**，Node 原生 `vm` 直接跑，`Buffer`、`node:crypto`、`zlib` 全有原生实现，一个垫片都不用——同样的 21 个脚本 **21/21 全部装载**。前端调**同源**的 `/api/url`，不需要额外域名、证书、CORS 配置，也不用再养一台服务器；直链缓存放 CDN 边缘（`s-maxage=900`），命中缓存的请求根本不进函数。

### 7.2 三个坑

**① 判「成功」要判到最终效果。** 最初的成功判据只是"返回了 http 开头的字符串"，于是"最快返回、但返回的是自己那套解析接口地址"的音源（有的甚至不联网、1ms 就返回）必然在竞速里胜出；一旦它指向的域名失效（实测 NXDOMAIN），**每首歌拿到的都是死链**，而死链不计入失败率，坏源永远进不了熔断——现场就是"接口全 200、音源全绿、直链全死"。修法是：拿到直链后真去拉一次（HEAD，不支持再 `Range: bytes=0-1`），要求 200/206 且 `Content-Type` 是 `audio/*`（JSON/HTML 一律判失败），**只有验证通过才算这个音源成功**。同一类错误还有个变体：竞速输掉、被取消的尝试不能记成失败，否则健康音源会被误熔断。

**② 好链接不能和坏链接一起竞速。** 代理直链一度是"插到候选链第一位、再和 Meting 一起探针竞速"，结果 Meting 的试听片段（能播）经常抢跑。现在的语义是：**解析到直链就直接播它**，Meting 整条链退居降级位，**只有这条直链失效才轮到 Meting**。

**③ 音源要按成绩用，不能全打一遍。** 音源一多，每次取流都向上游发 N 个请求，很容易被限流封 IP。调度改成：按成功率/耗时排序 → 一次并发 3 个 → 300ms 没结果再放一波 → **赢家一出立刻掐断其余在途请求** → 连续失败 3 次熔断 10 分钟。实测同一批歌：全打一遍约 3 秒、21 个并发请求；现在单曲 50~390ms、正常只有 3 个并发，坏源熔断后稳定在 50~70ms。

### 7.3 机房 IP 能不能用国内接口

这是上线前最大的未知数：函数跑在境外（默认 `hkg1` 香港），音乐接口都在国内。实测同一批 10 首歌：
...
完全一致，**连失败的都是同样两首**（歌本身没资源，跟地域无关）。中间有过一次"机房 0/8"的误判，事后查明当时只放了一个走网易官方接口的示例音源（它本来就拿不到 VIP 曲），跟 IP 无关——**结论要拿对照实验说话，别用单点数据下判断**。

### 7.4 开源与隐私的取舍

这套东西开源了（[Zlion-Y/zlion-homepage](https://github.com/Zlion-Y/zlion-homepage)），但**公开仓库里不放任何第三方音源脚本**：它们是别人的作品，也可能带着作者接口里的密钥。做法是公开仓只留示例脚本、用 `.gitignore` 挡住 `sources/*.js`；自己的音源要么放私有仓，要么托管在别处用 `SOURCE_URLS` 环境变量（`api/` + `lib/` 支持在线热换，改脚本连重新部署都不用）。
...
## 九、性能治理（一）：410 行歌单与 100ms 主线程长任务
...
## 十、性能治理（二）：GPU 拉满——模糊不贵，模糊的「背景在变」才贵
...
## 十一、性能治理（三）：毛玻璃的两个反直觉问题

### 11.1 毛玻璃慢半拍：opacity 动画会暂停 backdrop-filter
...
### 11.2 两层卡片「交叉抖动」：问题是采样哪一层，不是能不能看见
...
后来一轮反馈集中在移动端，都是"看着别扭"级别的问题，但每个背后都有确切的成因：

**① 面板滚到下半屏就没有壁纸了。** 面板在手机上自身可滚（`.more { overflow-y: auto }`），而壁纸层 `.more::before` 用的是 `position: absolute`——**它会跟着内容一起滚走**。壁纸层高度只有一个视口，所以滚过一屏之后就只剩面板底色。改成 `position: fixed` 常驻视口即解决（桌面端面板不滚，两种定位等价）。

**② 按钮行和进度条那一行没对齐。** 这里我先理解偏了一次：把"对齐进度条"做成了按钮两端对齐**进度条本体**内侧。实际要的是：随机播放 / 上一首 / 播放 / 下一首 / 列表这五个按钮**拉开到整行宽度**，两端与两侧时间的边缘竖向对齐。还有个细节——每个按钮是 34px 的盒子装 24px 图标，图标居中自带 5px 内缩，所以容器要**反向缩 5px**，图标边缘才真正落在时间边缘上（只对齐盒子的话视觉上仍然缩进 5px）。实测：左时间边缘 26 / 首个图标 27，右时间 364 / 末个图标 363。

**③ 按钮行离进度条太近、播放键又偏小。** 垂直间距从 14px 提到 28px（实测综合 32px），播放键在移动端放大到 56×56、图标 24→30px（桌面保持 FluentPlayer 的 42px 规格）。

这一轮最值得记的一条：**移动端的覆盖规则必须写在样式表末尾**。我第一版把它们塞进文件靠前的 `@media` 里，结果一条都没生效——基础规则（`.fs-play-btn { width: 42px }`）定义在更后面，同优先级下**后写的赢**。挪到末尾就好了，这类"写了却没生效"的问题，先看源码顺序，再看优先级。
...
接了洛雪音源之后，部署形态变成"**静态站 + 一个 serverless 函数**"：

- `api/url.mjs` + `lib/*` 跟主页一起部署，Vercel 按根目录 `api/` 约定打包成函数；`vercel.json` 里配了 `maxDuration: 60` 和 `includeFiles: sources/**`（音源脚本要打进函数包），区域选 `hkg1`（离国内接口最近）；
- 前端调**同源** `/api/url`，所以没有跨域、没有第二个域名、没有证书要管；
- 免费版（Hobby）的额度对这个用量绰绰有余：每月 100 万次调用、4 CPU-小时、100GB 出站流量，而**音频字节不经过 Vercel**（前端直连平台 CDN），直链又有 15 分钟 CDN 缓存——命中缓存的请求根本不进函数。真正要留意的是**运行时日志只保留 1 小时**，排查问题要当场看。

仓库也做了拆分：`zlion-home`（私有，放自己的音源脚本与配置）、`zlion-homepage`（公开，脱敏模板 + 示例脚本，默认走 Meting）。**第三方音源脚本不进公开仓库**，见 7.4。
...
5. **判「成功」要判到最终效果**。这一轮最贵的一次事故是：音源接口全都返回 200、音源状态全绿，但每首歌拿到的直链都是死的——因为成功判据只是"返回了一个 http 开头的字符串"。判中间形态（返回了字符串、HTTP 200）会漏掉一整类失败，要判就判"能不能拉到音频字节"。见 7.2。
6. **同一类错误会换一种形态再来一次**。"毛玻璃容器上不能做透明度动画"这条我在卡片上记住了，却在面板容器上又踩了一遍——把结论抽象成规则（"任何包含 backdrop-filter 的子树"），比记住具体案例更管用。
