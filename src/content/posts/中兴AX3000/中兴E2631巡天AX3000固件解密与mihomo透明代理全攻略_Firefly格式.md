...
**旧设备（192.168.5.22，GitHub 大文件）**：

| 指标 | VMess 节点（旧） | SS 节点（新） | 说明 |
|------|--------|-----------|------|
| 单线程 | **28.6 Mbps** | **19.8 Mbps** | 单连接 TCP 限制，与 CPU 无关 |
| 8 路并发 | **54.6 Mbps** | **35.0 Mbps** | 单线程实现，CPU 满载时的极限吞吐 |
| 并发增益 | 1.9x | 1.8x | 多线程可吃满单核 |
| 相对 VMess | 基准 | -36% | 新 SS 节点整体更慢 |

**新设备（192.168.5.11，IEPL 节点）**：

| 指标 | Google 源（dl.google.com） | GitHub 源 | 说明 |
|------|--------|-----------|------|
| 单线程 | **91.9 Mbps** | 32.6 Mbps | 多线程 mihomo，双核均可参与 |
| 8 路并发 | **79.0 Mbps** | 44.7 Mbps | 持续压测采样峰值 **140-165 Mbps** |
| 结论 | IEPL 节点不限速 | ~66 Mbps 是「节点→GitHub CDN」线路差 | **测速源选择决定结论**，Google 系源才反映真实带宽 |

> [!IMPORTANT] 重要
> 同节点用 GitHub 源测只有 ~44-66 Mbps，换 Google 官方源（dl.google.com / storage.googleapis.com）稳定 140-165 Mbps——**测速源选择决定结论**，代理带宽要用 Google 系源测，才贴近 Play 商店真实下载场景。新设备吞吐远超旧设备单核极限（~55 Mbps），瓶颈在代理实现能否用上双核，而非内核路径或内存。


> [!NOTE] 提示
> 旧设备日常用多线程下载工具（IDM/aria2）即可吃满 ~55 Mbps；单线程应用受 TCP 单连接限制只有 ~28 Mbps。VMess 的 `cipher: auto` 在无硬件 AES 的 A9 上实际使用 chacha20-ietf-poly1305（纯软件），比 SS 的软浮点 AES-256-GCM 更省 CPU——这也是 SS 节点并发吞吐低 36% 的原因之一。

### 6.2 CPU 单核瓶颈判定

| 状态 | CPU | 代理进程 | Load |
|------|-----|---------|------|
| 压测满载（top 快照） | usr 37.5% + sirq 12.5% · idle 50% | **45.7%** | 2.07 |
| 整周期采样（/proc/stat） | **cpu0 100% 持续 ~240s / cpu1 全程 0%** ← 单核瓶颈实锤 | - | **4.04**（峰值） |
| 彻底空载 | idle 100% | 0% | 0.60 |
| 内存 | 压测中 MemFree 最低 **10.9MB**（常态 14-17MB），全程无压力 | - | - |

- **旧设备（.22）单线程实现**：代理进程单线程 + 网卡软中断全部绑定核 0，核 1 完全闲置，load1 峰值 4.04（排队全挤单核）——8 路并发极限 54.6 Mbps，**瓶颈是单核算力**；
- **新设备（.11）多线程 mihomo**：双核均可参与，IEPL 节点 + Google 源实测 140-165 Mbps——**瓶颈在代理实现能否用上双核，不在内核路径/内存**；
- 中兴固件未开放 RPS 中断分散，单核被软中断压满时固件层面无解；
- 判定 CPU 瓶颈看 `/proc/stat` 的 cpu0/cpu1 差分与 top 的 `idle%/sirq%`，不要看 loadavg（含 D 状态任务的水分，空载时 load 仍可能高达 2+）；
